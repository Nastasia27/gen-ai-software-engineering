...
- [x] 4.0.1 `python3 --version`
- [x] 4.0.2 `uv --version`; if missing — `brew install uv`
- [x] 4.0.3 Create the folder: `mkdir -p custom-mcp-server`

### 4.1 Server files
- [x] 4.1.1 `custom-mcp-server/lorem-ipsum.md` — source text (> 30 words).
- [x] 4.1.2 `custom-mcp-server/requirements.txt` with the line `fastmcp` (or `pyproject.toml`).
- [x] 4.1.3 `custom-mcp-server/server.py` using FastMCP:
  - [x] **Resource** with a URI (e.g. `lorem://words`), reads `lorem-ipsum.md`, accepts
        `word_count` (default `30`), returns exactly that many words.
  - [x] **Tool** `read(word_count: int = 30)` — returns the content from the same resource.

### 4.2 Install and run locally
- [x] 4.2.1 Install dependencies: `cd custom-mcp-server && uv pip install -r requirements.txt`
- [x] 4.2.2 Verify the server starts without errors: `uv run server.py`

### 4.3 Connect to Claude
- [x] 4.3.1 Add the custom server:
  ```bash
  ...
  ```
- [x] 4.3.2 Restart Claude Code, `/mcp` → `custom-lorem` **connected**.
  📸 `docs/screenshots/custom-mcp-connected.png` — `read` tool visible in `/mcp`.

### 4.4 Test the `read` tool
- [x] 4.4.1 Request: > "Use the custom-lorem MCP `read` tool to return 10 words."
- [x] 4.4.2 Request: > "Now call `read` with the default word_count and show the result."
  (verify default = 30 words, and the parameter limits the count).
- [x] 4.4.3 📸 `docs/screenshots/custom-mcp-read-tool-result.png` — `read` call + result.
...
- [x] 5.1 `homework-5/README.md`:
  - [x] description of the work + **author name**;
  - [x] explanation: **Resources** = URIs that Claude *reads* (files, APIs); **Tools** = actions
        that Claude *calls* (read a file, run a command).
- [x] 5.2 `homework-5/HOWTORUN.md`: install dependencies → run the server →
...
- [x] 6.1 All **4** servers registered in `.mcp.json` (github, filesystem, jira/notion, custom-lorem).
- [x] 6.2 All 4 screenshots in place in `docs/screenshots/`:
  - [x] `github-mcp-result.png`
  - [x] `filesystem-mcp-result.png`
  - [x] `jira-or-notion-mcp-result.png`
  - [x] `custom-mcp-read-tool-result.png`
- [x] 6.3 `fastmcp` explicitly present in `requirements.txt` / `pyproject.toml`.
- [ ] 6.4 Folder structure matches `TASKS.md`.
- [x] 6.5 ⚠️ NO tokens in commits. Add `.gitignore`, keep real tokens in env
      or in `.mcp.json` that is not committed.
      → verified: no hardcoded secrets in any tracked file; `.mcp.json` uses
        `${GITHUB_PERSONAL_ACCESS_TOKEN}` + Atlassian OAuth; root `.gitignore` covers `.env`.
- [x] 6.6 Open a PR with summary and embedded screenshots; README with author name.
